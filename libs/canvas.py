...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        if self.drawing():
            ...
            if self.current:
                ...
                if self.outOfPixmap(pos) and (self.phase != 0):
        ...
        if Qt.LeftButton & ev.buttons():
            if self.selectedVertex():
                self.boundedMoveVertex(pos, self.phase != 0)
    ...
    def boundedMoveVertex(self, pos, rect=True):
        index, shape = self.hVertex, self.hShape
        point = shape[index]
        if self.outOfPixmap(pos) and rect:
    ...
    def paintEvent(self, event):
        ...
        if self.current is not None and len(self.line) == 2:
            ...
            print(2)
            # polygon = QPolygonF()
            # polygon.append(QPointF(leftTop.x(), leftTop.y()))
            # polygon.append(QPointF(leftTop.x() + rectWidth, leftTop.y() + rectWidth))
            # polygon.append(QPointF(leftTop.x(), leftTop.y() + rectWidth))
    ...
    def finalise(self):
        ...
        # s = Shape()
        # s.addPoint(self.current.points[0])
        # s.addPoint(self.current.points[2])
        # s.addPoint(self.current.points[1])
        # s.addPoint(self.current.points[3])
    ...
    def keyPressEvent(self, ev):
        ...
        elif self.phase == 1 and key == Qt.Key_Right and modifiers != Qt.ControlModifier and self.selectedShape:
            idx_selected = self.get_index_selected_shape()
            idx_selected = (idx_selected + 1) % len(self.shapes)
            self.selectShape(self.shapes[idx_selected])
        elif self.phase == 1 and key == Qt.Key_Left and modifiers != Qt.ControlModifier and self.selectedShape:
            idx_selected = self.get_index_selected_shape()
            idx_selected = (idx_selected - 1 + len(self.shapes)) % len(self.shapes)
            self.selectShape(self.shapes[idx_selected])

        elif key == Qt.Key_Left and modifiers == Qt.ControlModifier and self.selectedShape:
            self.moveOnePixel('Left')
        elif key == Qt.Key_Right and modifiers == Qt.ControlModifier and self.selectedShape:
            self.moveOnePixel('Right')
        elif key == Qt.Key_Up and modifiers == Qt.ControlModifier and self.selectedShape:
